...
contract SwapPairExplorer is Debot, Upgradable, TIP3WalletsDatabase {
    ...
    function getDebotInfo() public functionID(0xDEB) view returns(
        ...
        name      = "TonSwap Liquidity withdrawing debot";
        version   = "0.1.0";
    ...
    function getVersion() public override returns (string name, uint24 semver) {
        (name, semver) = ("TonSwap Liquidity withdrawing debot", _version(0, 1, 0));
    }
    ...
    function mainMenu(uint32 index) public {
        MenuItem[] mi;
        mi.push(MenuItem("Withdraw liquidity", "", tvm.functionId(withdrawLiquidityEntryPoint)));
        mi.push(MenuItem("Add token wallets", "Add token wallets for iteration with swap pair and get their status", tvm.functionId(addWalletEntryPoint)));
    ...
    function getAmountForLPToken() public {
        AmountInput.get(
    ...
    function getPayloadForLiquidityProviding() public view {
        optional(uint256) pubkey;
        ISwapPair(currentSwapPair).createWithdrawLiquidityPayload{
            abiVer: 2,
            extMsg: true,
            sign: false,
            ...
            spi.tokenRoot1, tip3Wallets[spi.tokenRoot1].wallet_address,
            spi.tokenRoot2, tip3Wallets[spi.tokenRoot2].wallet_address
        );
    }

    function receivePayloadForLPWithdrawing(TvmCell payload_) public {
        Terminal.print(0, format("Sending {} tokens to swap pair LP wallet", lpTokenAmount));
        sendMessageToTIP3(spi.lpTokenRoot, spi.lpTokenWallet, lpTokenAmount, payload_);
    ...
    function sendMessageToTIP3(address rootAddress, address sendTokensTo, uint128 amount, TvmCell payload_) public view {
        if (tip3Wallets[rootAddress].manageType == ManageType.MANAGE_WITH_KEYPAIR) {
            ...
            ITIP3Token(tip3Wallets[rootAddress].wallet_address).transfer{
                ...
                sendTokensTo, amount, 1 ton, address.makeAddrStd(0, 0), true, payload_
            );
        } else {
            TvmCell payloadForTIP;
            payloadForTIP = tvm.encodeBody(
                ITIP3Token.transfer, 
                sendTokensTo, amount, 0.8 ton, address.makeAddrStd(0, 0), true, payload_
            );
            optional(uint256) pubkey = 0;
            IMultisig(tip3Wallets[rootAddress].owner_address).sendTransaction{
                ...
                expire: 0,
                pubkey: pubkey
            }(tip3Wallets[rootAddress].wallet_address, 1 ton, true, 1, payloadForTIP);
        }
    }
