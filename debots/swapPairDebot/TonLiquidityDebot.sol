...
contract SwapPairExplorer is Debot, Upgradable, TIP3WalletsDatabase {
    ...
    function mainMenu(uint32 index) public {
        MenuItem[] mi;
        mi.push(MenuItem("Swap tokens", "", tvm.functionId(provideLiquidityEntryPoint)));
    ...
    function confirmNoLPWallet(bool value) public {
        ...
        if (!value) {
            Terminal.print("Add wallet for LP token please");
            addTIP3WalletEntryPoint(0);
        }
    ...
    function getPayloadForLiquidityProviding() public {
        optional(uint256) pubkey;
    ...
    function sendMessageToTIP3(address rootAddress, uint128 amount, TvmCell payload_) public {
        if (tip3Wallets[rootAddress].manageType == ManageType.MANAGE_WITH_KEYPAIR) {
            ...
            ITIP3Token(tip3Wallets[rootAddress].wallet_address).transfer{
                ...
                tip3Wallets[rootAddress].wallet_address, amount, 0.2 ton, address.makeAddrStd(0, 0), true, payload_
            );
        } else {
            TvmCell payloadForTIP;
            payloadForTIP = tvm.encodeBody(
                ITIP3Token.transfer, 
                rootAddress == spi.tokenRoot1 ? spi.tokenWallet1 : spi.tokenWallet2, 
                amount, 0.2 ton, address.makeAddrStd(0, 0), true, payload_
            );
            optional(uint256) pubkey = 0;
